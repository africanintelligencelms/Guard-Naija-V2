...
export const fetchLiveSecurityNews = async (): Promise<SecurityNewsItem[]> => {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      contents:
        "Find the latest 3 major security incidents or news reports in Nigeria from the last 24 hours. Focus on kidnapping, banditry, or government security operations. Provide a headline and the specific news source name for each.",
      config: {
        tools: [{ googleSearch: {} }],
      },
    });
