...
import { Activity, ShieldAlert, CheckCircle, MapPin, Radio } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const recentIncidents = [...incidents].sort((a, b) => b.timestamp - a.timestamp).slice(0, 5);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 transition-transform hover:-translate-y-1">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">Critical Reports</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col">
          <div className="flex justify-between items-center mb-4">
             <h3 className="text-lg font-bold text-gray-900">Live Incident Map</h3>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Reports by Category</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
          <h3 className="text-lg font-bold text-gray-900">Recent Public Reports</h3>
          ...
          {recentIncidents.length > 0 ? (
            recentIncidents.map((incident) => (
              <div key={incident.id} className="p-6 hover:bg-gray-50 transition-colors group">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        incident.type === IncidentType.Kidnapping ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'
                      }`}>
                        {incident.type}
                      </span>
                      {incident.isAnonymous && (
                          <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded border border-gray-200">Anon</span>
                      )}
                  </div>
                ...
                <p className="text-gray-800 font-medium group-hover:text-guard-green transition-colors">{incident.description}</p>
                <div className="mt-3 flex items-center gap-2 text-sm text-gray-500">
                  <MapPin className="h-4 w-4 text-gray-400" />
                  <span className="truncate max-w-md">{incident.location?.address || 'Unknown Location'}</span>
                </div>
              </div>
            ))
          ) : (
