...
import { 
  Shield, AlertTriangle, CheckCircle, TrendingUp, Users, FileText, 
  Eye, Download, Clock, Lock, Globe, ExternalLink, Search, ChevronDown, RefreshCw
} from 'lucide-react';
import { IncidentStatus, SeverityLevel, SecurityNewsItem } from '../types';
...
const MOCK_LOGS: LogEntry[] = [
  { id: '1', action: 'Status Update', details: 'Changed Incident #8392 to "Resolved"', role: 'Admin (Lagos)', timestamp: Date.now() - 1000 * 60 * 5, type: 'edit' },
  { id: '2', action: 'Data Export', details: 'Downloaded monthly security report CSV', role: 'Super Admin', timestamp: Date.now() - 1000 * 60 * 45, type: 'system' },
  { id: '3', action: 'Sensitive View', details: 'Accessed detailed report for Incident #1102', role: 'Admin (Abuja)', timestamp: Date.now() - 1000 * 60 * 60 * 2, type: 'view' },
  { id: '4', action: 'Login Attempt', details: 'Failed login attempt from IP 192.168.1.1', role: 'System', timestamp: Date.now() - 1000 * 60 * 60 * 5, type: 'security' },
  { id: '5', action: 'Status Update', details: 'Changed Incident #2291 to "In Progress"', role: 'Admin (Kano)', timestamp: Date.now() - 1000 * 60 * 60 * 24, type: 'edit' },
];
...
export const AdminDashboard: React.FC = () => {
  ...
  const trendData = sortedIncidents.map((inc, i) => ({
    time: new Date(inc.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    total: i + 1,
    critical: sortedIncidents.slice(0, i + 1).filter(x => x.severity === SeverityLevel.Critical).length
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      {/* Header Stats */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-6">National Security Overview</h2>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                />
                <Area type="monotone" dataKey="total" stroke="#008751" strokeWidth={2} fillOpacity={1} fill="url(#colorTotal)" name="All Incidents" />
                <Area type="monotone" dataKey="critical" stroke="#DC2626" strokeWidth={2} fillOpacity={1} fill="url(#colorCritical)" name="Critical" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              <PieChart>
                ...
                >
                  {statusData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={STATUS_COLORS[entry.name as IncidentStatus] || COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[500px]">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
             <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900">Live Media Intelligence</h3>
             </div>
             <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">Powered by Gemini</span>
          </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[500px]">
           <div className="p-4 border-b border-gray-100 flex justify-between items-center">
              <h3 className="font-bold text-gray-900">Geospatial Threat Heatmap</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
         <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
            <h3 className="text-lg font-bold text-gray-900">Full Incident Database</h3>
               ...
               <tbody className="divide-y divide-gray-100">
                  {filteredIncidents.slice(0, 10).map((incident) => (
                     <tr key={incident.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 font-mono text-xs text-gray-500">#{incident.id.substring(0, 6)}</td>
                        <td className="px-6 py-4 font-medium text-gray-900">{incident.type}</td>
                        <td className="px-6 py-4 text-gray-500 truncate max-w-[200px]">{incident.location?.address}</td>
                        <td className="px-6 py-4">
                           <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              incident.severity === SeverityLevel.Critical ? 'bg-red-100 text-red-700' :
                              incident.severity === SeverityLevel.High ? 'bg-orange-100 text-orange-700' :
                        ...
                        <td className="px-6 py-4">
                           <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                              incident.status === IncidentStatus.Resolved ? 'bg-green-50 text-green-700 border-green-200' : 
                              'bg-gray-50 text-gray-700 border-gray-200'
                           }`}>
                              {incident.status}
                           </span>
                        </td>
                        <td className="px-6 py-4 text-gray-500">{new Date(incident.timestamp).toLocaleDateString()}</td>
                ...
                <div key={log.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-4">
                        <div className={`p-2 rounded-lg ${
                            ...
                            log.type === 'edit' ? 'bg-orange-50 text-orange-600' :
                            log.type === 'system' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-600'
                        }`}>
                            {log.type === 'security' ? <Shield className="h-4 w-4" /> :
                             log.type === 'edit' ? <RefreshCw className="h-4 w-4" /> :
                             log.type === 'system' ? <Download className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                        </div>
                        <div>
                    ...
                    <div className="text-right">
                        <p className="text-xs font-medium text-gray-900">{log.role}</p>
