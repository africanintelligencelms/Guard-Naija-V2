...
export const AgencyDashboard: React.FC = () => {
  ...
  const getStatusColor = (status: IncidentStatus) => {
    switch (status) {
      case IncidentStatus.Submitted: return 'bg-gray-100 text-gray-800 border-gray-200';
      case IncidentStatus.Verified: return 'bg-blue-100 text-blue-800 border-blue-200';
      case IncidentStatus.InProgress: return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case IncidentStatus.Resolved: return 'bg-green-100 text-green-800 border-green-200';
  ...
  const getSeverityColor = (severity: SeverityLevel) => {
    switch (severity) {
      case SeverityLevel.Critical: return 'text-red-600 font-bold';
      case SeverityLevel.High: return 'text-orange-600 font-semibold';
      case SeverityLevel.Medium: return 'text-yellow-600';
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Agency Command Center</h2>
            <p className="text-sm text-gray-500">Monitor and manage security incident reports.</p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 border-b border-gray-100 mb-4 w-full">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
              filterStatus === 'All' ? 'bg-gray-900 text-white shadow-md' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-200'
            }`}
          >
            All Reports
          </button>
          {Object.values(IncidentStatus).map(status => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
                filterStatus === status ? 'bg-guard-green text-white shadow-md' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-200'
              }`}
            >
              {status}
            </button>
                ...
                filteredIncidents.map((incident) => (
                  <tr key={incident.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-4 align-top">
                      ...
                      <div className="text-xs text-gray-500 font-mono mt-1">ID: {incident.id.substring(0, 8)}</div>
                    </td>
                    <td className="px-4 py-4 align-top">
                      <div className="flex items-start gap-1.5 text-gray-700">
                        <MapPin className="h-3.5 w-3.5 text-gray-400 mt-0.5 flex-shrink-0" />
                        <span className="truncate max-w-[200px] block" title={incident.location?.address}>
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className={`flex items-center gap-1.5 ${getSeverityColor(incident.severity)}`}>
                        <AlertTriangle className="h-4 w-4" />
                        {incident.severity}
                      </div>
                    </td>
                    <td className="px-4 py-4 align-top">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(incident.status)}`}>
                        {incident.status}
                      </span>
                    </td>
        ...
        <div className="md:hidden space-y-4">
           {filteredIncidents.length > 0 ? (
                filteredIncidents.map((incident) => (
                  <div key={incident.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-3 active:scale-[0.99] transition-transform">
                    <div className="flex justify-between items-start">
                        <div>
                             <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider mb-1 ${getSeverityColor(incident.severity)} bg-gray-50 border border-gray-100`}>
                                {incident.severity}
                             </span>
                             <h3 className="font-bold text-gray-900">{incident.type}</h3>
                        </div>
                        <span className={`px-2 py-1 rounded text-[10px] font-bold border uppercase tracking-wide ${getStatusColor(incident.status)}`}>
                            {incident.status}
                        </span>
                    </div>
